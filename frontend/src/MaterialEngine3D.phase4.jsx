...
export default function MaterialEngine3D({ material = "denim", exploded = false, onPartSelect }) {
  ...
  useEffect(() => {
    ...
    // Load the committed Higgsfield material PNGs from Vite's public directory.
    // These are the actual textures used by the mesh materials below.
    const textures = {};
    const loader = new THREE.TextureLoader();
    const materialTexturePromises = Object.keys(MATERIALS).map((key) => new Promise((resolve, reject) => {
      loader.load(
        `/materials/${key}.png`,
        (tex) => {
    ...
    // Once the four local PNGs are loaded, bind the active one to every part.
    Promise.all(materialTexturePromises).then(() => {
      applyMaterial(stateRef.current.material);
    }).catch((error) => {
      console.error("MaterialEngine3D: failed to load local material texture", error);
    });
