...
/* ---------- Brand intro — a few seconds of atmosphere before the
   ...
   Self-hosted <video> (not the old Descript iframe): the iframe had
   no real fail path, so a slow/blocked embed just sat on a broken
   frame for the whole window. Now onError skips straight to the
   landing page, and a hard safety timeout fires regardless of
   whether the video's own events ever fire at all. ---------- */
function IntroOverlay({ onDone }) {
  ...
  useEffect(() => {
    // Second line of defense — fires regardless of whether the video's
    // onEnded/onError events ever fire (e.g. it stalls mid-load without
    // erroring outright). Nothing should be able to hang past this.
    const safetyTimer = setTimeout(finish, 4200);
    return () => clearTimeout(safetyTimer);
  }, [finish]);

  const handleError = () => {
    // Missing/broken asset — fail invisibly instead of visibly: skip
    // straight to the landing page rather than showing a broken-frame
    // icon for the rest of the intro window.
    setVideoFailed(true);
    finish();
  };
  ...
  return (
    ...
    >
      <video
        ...
        onEnded={finish}
        onError={handleError}
        className={`h-full w-full object-cover transition-opacity duration-500 ${phase === "fading" ? "opacity-0" : "opacity-100"}`}
